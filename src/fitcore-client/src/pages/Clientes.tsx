...
export default function Clientes() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5192/api/clientes/${editing.id}`,
        {
          method: "PUT",
  ...
  const handleToggleActivo = async (c: Cliente) => {
    ...
    try {
      if (c.activo) {
        const res = await fetch(
          `http://localhost:5192/api/clientes/${c.id}`,
          { method: "DELETE" }
        );
        if (!res.ok) throw new Error("DELETE failed");

        setClientes((prev) =>
          prev.map((x) => (x.id === c.id ? { ...x, activo: false } : x))
        );
        toast({
          variant: "success",
          title: "Cliente desactivado",
          description: "El cliente quedó inactivo.",
        });
      } else {
        ...
        setClientes((prev) =>
          prev.map((x) => (x.id === c.id ? { ...x, activo: true } : x))
        );
        toast({
          variant: "success",
          title: "Cliente reactivado",
          description: "El cliente quedó activo.",
        });
      }
    } catch {
      toast({
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell
                    colSpan={6}
                    className="text-center text-gray-500 py-8 font-normal"
                  >
                    No hay clientes
                  </TableCell>
                </TableRow>
                ...
                clientesFiltrados.map((c) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {new Date(c.fechaAlta).toLocaleDateString()}
                    </TableCell>
                    <TableCell>
